...
exports.maybeParseAsChunkFrame = maybeParseAsChunkFrame;
const XOR_ZERO = 48; // Shifts characters codes down by 48 so "0" is  0
const PIPE_XZ = 76; // Shifted "|"'s character code
/**
 ...
 */
function maybeParseStringAsChunkFrame(input, length) {
    // parse id
    ...
    // parse fin
    const fin = ((input.charCodeAt(i) ^ XOR_ZERO) | 0);
    if (fin >>> 1 !== 0) {
        return null;
    }
 ...
 * @returns a {@link ChunkFrame} if the value is a valid chunk frame, otherwise null
 */
function maybeParseAsChunkFrame(input) {
