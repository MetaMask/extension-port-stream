 ...
 *
 * @param {Workspace} workspace - The workspace to check.
 * @param {string} packageName - The name of the workspace package.
 * @returns {Promise<void>}
 */
async function expectReadme(workspace, packageName) {
  const readme = await getWorkspaceFile(workspace, 'README.md');
  if (
    packageName !== '@metamask/metamask-module-template' &&
...
function expectExports(workspace) {
  ...
  if (!manifestExports) {
    workspace.error("There are no exports defined in package.json.");
    return;
  }
...
module.exports = defineConfig({
  async constraints({ Yarn }) {
    ...
    // TODO: Update when we rename the package
    ...
    await expectReadme(workspace, workspace.manifest.name);
    ...
    // TODO: Re-enable when we change the supported version of Node
    // See: https://github.com/MetaMask/metamask-module-template/pull/271
    ...
    // TODO: Re-enable when we switch to ts-bridge
